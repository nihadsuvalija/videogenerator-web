...
import { RefreshCw, Download, AlertCircle, Check, Clock, ChevronDown, Pencil } from 'lucide-react';
          ...
          <div className="divide-y divide-border">
            {jobs.map(job => (
              <div key={job.id} className="px-6 py-3">
                <div className="flex items-center gap-3 cursor-pointer" onClick={() => setExpanded(expanded === job.id ? null : job.id)}>
                  {statusIcon(job.status)}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold font-mono">{job.batchName}</span>
                      <span className={cn("text-xs px-1.5 py-0.5 rounded-full", statusLabel[job.status])}>{job.status}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                    {job.status === 'done' && job.outputFile && (
                      <>
                        ...
                        <a
                          href={`${API}/outputs/${job.outputFile}`}
                          download
